...
import { Users, Megaphone, MessageCircle, UserPlus, CheckCircle2, Clock3, Pause, Play, Send, UserRound, CalendarDays } from "lucide-react";
...
function interpolate(template: string, recipient: Recipient, clinicName: string) {
  return template
    .replaceAll("{{patient_name}}", recipient.full_name || "there")
    .replaceAll("{{clinic_name}}", clinicName)
    .replaceAll("{{campaign_date}}", "");
}
...
export default function Outreach() {
      ...
      if (audience === "external" || audience === "both") {
        ...
        for (const raw of externalText.split(/\n|,/)) {
          ...
          const { data: contact } = await apiClient.from("outreach_contacts").upsert({
            clinic_id: effectiveClinicId, full_name: fullName, phone, normalized_phone: normalized, source: "campaign",
          }, { onConflict: "clinic_id,normalized_phone" }).select("id").single();
          rows.push({ campaign_id: campaign.id, contact_id: contact?.id || null, full_name: fullName, phone, normalized_phone: normalized, status: "ready" });
        }
      }
      if (rows.length) {
  ...
  const openNext = async () => {
    ...
    setSending(true);
    try {
      const url = whatsappLink(current.phone, interpolate(selected.message_template, current, clinicName));
      if (!url) return;
    ...
    <div className="space-y-5">
      ...
      {showCreate && <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4"><div className="w-full max-w-2xl max-h-[90vh] overflow-auto rounded-2xl bg-card border shadow-2xl p-5">
        ...
        <div className="grid gap-4 mt-5">
          ...
          <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setShowCreate(false)}>Cancel</Button><Button onClick={() => void createCampaign()} disabled={creating}>{creating ? "Creating…" : "Create campaign"}</Button></div>
        </div>
      </div></div>}
    </div>
  );
