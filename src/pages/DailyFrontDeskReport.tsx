...
type PatientRow = {
  ...
  hmo_request_sent: boolean;
  hmo_request_sent_at: string | null;
  hmo_request_status: string | null;
  hmo_request_response_at: string | null;
...
export default function DailyFrontDeskReport() {
    ...
    try {
      ...
      const merged: PatientRow[] = asArray<any>(patientsRes.data).map((p: any) => {
        ...
        return {
          ...
          hmo_request_sent: !!claimsByPatient.get(p.patient_id)?.hmo_request_sent,
          hmo_request_sent_at: claimsByPatient.get(p.patient_id)?.hmo_request_sent_at ?? null,
          hmo_request_status: claimsByPatient.get(p.patient_id)?.hmo_request_status ?? saved?.hmo_claim_status ?? (p.patient_type === "hmo" ? "Not sent" : null),
          hmo_request_response_at: claimsByPatient.get(p.patient_id)?.hmo_request_response_at ?? null,
          hmo_request_remarks: claimsByPatient.get(p.patient_id)?.hmo_request_remarks ?? saved?.hmo_claim_remarks ?? claimsByPatient.get(p.patient_id)?.notes ?? null,
  ...
  async function saveHmoClaim(row: PatientRow) {
    ...
    const { error } = await db.from("hmo_claims").update({
      hmo_request_sent: row.hmo_request_sent,
      hmo_request_sent_at: row.hmo_request_sent ? (row.hmo_request_sent_at || new Date().toISOString()) : null,
      hmo_request_status: row.hmo_request_status || "Not sent",
      hmo_request_response_at: row.hmo_request_status && row.hmo_request_status !== "Not sent" ? (row.hmo_request_response_at || new Date().toISOString()) : null,
      hmo_request_remarks: row.hmo_request_remarks || null,
...
function DesktopRow({ row, index, editable, saving, patch, save }: any) {
  return <tr className="border-t hover:bg-muted/20 align-middle">
    ...
    <td className="p-2 md:p-3">{row.patient_type === "hmo" ? <div className="space-y-1.5"><label className="flex items-center gap-2 text-[11px]"><input type="checkbox" checked={row.hmo_request_sent} disabled={!editable} onChange={(e) => patch(row.key,{hmo_request_sent:e.target.checked, hmo_request_status:e.target.checked ? "Sent" : "Not sent"})} /> Request sent</label><SelectStatus value={row.hmo_request_status || "Not sent"} options={HMO_REQUEST_STATUSES} disabled={!editable} onChange={(v) => patch(row.key,{hmo_request_status:v})} /></div> : "—"}</td>
<td className="p-2 md:p-3 whitespace-nowrap">{row.patient_type === "hmo" ? <strong>{money(row.hmo_amount_to_claim)}</strong> : "—"}</td>
<td className="p-2 md:p-3 min-w-[210px]">{row.patient_type === "hmo" ? <div className="space-y-1.5"><label className="flex items-center gap-2 text-[11px]"><input type="checkbox" checked={row.hmo_claim_sent} disabled={!editable} onChange={(e) => patch(row.key,{hmo_claim_sent:e.target.checked})} /> Claim sent</label><SelectStatus value={row.hmo_claim_response_status || "Pending"} options={HMO_RESPONSE_STATUSES} disabled={!editable} onChange={(v) => patch(row.key,{hmo_claim_response_status:v})} /><Input value={row.hmo_claim_response_remarks || ""} onChange={(e) => patch(row.key,{hmo_claim_response_remarks:e.target.value})} disabled={!editable} placeholder="Response / query / remarks" className="h-8 text-xs" /></div> : "—"}</td>
...
function MobileRow({ row, open, setOpen, editable, saving, patch, save }: any) {
  return <div className="p-3"><button type="button" onClick={setOpen} className="w-full text-left"><div className="flex items-center justify-between gap-3"><div><div className="font-semibold text-sm">{row.patient_name}</div><div className="text-[11px] text-muted-foreground">{row.patient_number || "No patient number"} · {row.patient_type === "hmo" ? row.hmo_name || "HMO" : "Private"}</div></div><div className="flex flex-wrap justify-end gap-1"><Status value={row.patient_type === "hmo" ? "HMO" : "Private"} />{row.feedback_follow_up !== "Not required" && <Status value={row.feedback_follow_up} />}</div></div></button>{open && <div className="mt-3 rounded-xl border bg-muted/20 p-3 space-y-3"><div className="grid grid-cols-2 gap-3 text-xs"><Info label="HMO Request" value={row.patient_type === "hmo" ? row.hmo_request_status || "Not sent" : "—"} /><Info label="Lens Type" value={row.lens_type || "—"} /><Info label="Glasses Prescription" value={row.glasses_prescription_sent ? "Sent" : row.prescription_available ? "Not sent" : "Not required"} /><Info label="Feedback" value={row.feedback_form_sent ? "Sent" : "Not sent"} /><Info label="Feedback Follow-up" value={row.feedback_follow_up} /><Info label="Medication" value={row.medication_name ? `${row.medication_name} · ${row.medication_dispensed ? "Dispensed" : "Not dispensed"}` : "—"} /></div>{editable && row.patient_type === "hmo" && <div className="space-y-2"><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={row.hmo_request_sent} onChange={(e) => patch(row.key,{hmo_request_sent:e.target.checked, hmo_request_status:e.target.checked ? "Sent" : "Not sent"})} /> Request sent</label><SelectStatus value={row.hmo_request_status || "Not sent"} options={HMO_REQUEST_STATUSES} onChange={(v) => patch(row.key,{hmo_request_status:v})} /><Info label="Amount to claim" value={money(row.hmo_amount_to_claim)} /><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={row.hmo_claim_sent} onChange={(e) => patch(row.key,{hmo_claim_sent:e.target.checked})} /> Claim sent</label><SelectStatus value={row.hmo_claim_response_status || "Pending"} options={HMO_RESPONSE_STATUSES} onChange={(v) => patch(row.key,{hmo_claim_response_status:v})} /><Input value={row.hmo_claim_response_remarks || ""} onChange={(e) => patch(row.key,{hmo_claim_response_remarks:e.target.value})} placeholder="Response / query / remarks" /></div>}{row.lens_order_required && editable && <SelectStatus value={row.lens_order_status || "pending"} options={LENS_ORDER_STATUSES} onChange={(v) => patch(row.key,{lens_order_status:v})} />}<Textarea value={row.remarks || ""} onChange={(e) => patch(row.key,{remarks:e.target.value})} disabled={!editable} placeholder="Remarks" className="min-h-[70px]" /><Button className="w-full" onClick={() => void save(row)} disabled={!editable || saving}>{saving ? <Loader2 size={14} className="mr-1 animate-spin" /> : <CheckCircle2 size={14} className="mr-1" />}Save Patient Row</Button></div>}</div>;
}
