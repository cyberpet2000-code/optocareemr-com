...
if ((billingMissing || 0) > 0) {
  console.warn(
    `[DIAG] billing missing clinic_id: ${billingMissing}`
      ...
      const patientStatsRes = await apiClient.rpc(
...
checkQueryFailure(
  "get_dashboard_patient_stats",
  "patient statistics",
  patientStatsRes.error
);

const patientStats = Array.isArray(patientStatsRes.data)
  ? patientStatsRes.data[0]
  : patientStatsRes.data;

const patientsSeenCount = Number(
  patientStats?.patients_seen ?? 0
);

const newPatientsSeenCount = Number(
  patientStats?.new_patients_seen ?? 0
);

const returningPatientsCount = Number(
  patientStats?.returning_patients ?? 0
);
      ...
      const snap: DashboardSnapshot = {
        monthPatients: patientsSeenCount,
patientsSeen: patientsSeenCount,
newPatientsSeen: newPatientsSeenCount,
