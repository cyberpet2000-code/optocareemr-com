...
async function removeQueueItemAndSave<T extends { [k:string]: any }>(key:string, queue:T[], idx:number): Promise<void> {
  queue.splice(idx,1);
  if (queue.length === 0) await secureOfflineRemove(key);
  else await secureOfflineSave(key, queue);
}

async function migrateLegacyQueues(clinicId: string): Promise<void> {
  const appointmentKey = appointmentsQueueKey(clinicId);
  const appointmentQueue = await secureOfflineGet<AppointmentQueueItem[]>(appointmentKey) ?? [];
  ...
  if (appointmentQueue.length) secureOfflineRemove(appointmentKey);

  const billKey = billsQueueKey(clinicId);
  const billQueue = await secureOfflineGet<BillQueueItem[]>(billKey) ?? [];
  ...
  if (billQueue.length) secureOfflineRemove(billKey);
}
...
export async function processAppointmentsQueue(clinicId: string): Promise<{ success:number; failed:number }> {
  ...
  let success = 0, failed = 0;
  try {
    const queue = (await secureOfflineGet<AppointmentQueueItem[]>(key) ?? []) as AppointmentQueueItem[];
    let idx = 0;
...
export async function processBillsQueue(clinicId: string, directItem?: BillQueueItem): Promise<{ success:number; failed:number }> {
  ...
  try {
    const queue = directItem
      ? [directItem]
      : ((await secureOfflineGet<BillQueueItem[]>(key) ?? []) as BillQueueItem[]);
    let idx = 0;
    while (idx < queue.length) {
      const item = queue[idx];
      try {
...
export async function runOfflineSync(clinicId: string): Promise<void> {
  ...
  try {
    ...
    const legacyAppointments = await secureOfflineGet<AppointmentQueueItem[]>(appointmentsQueueKey(clinicId)) ?? [];
    const legacyBills = await secureOfflineGet<BillQueueItem[]>(billsQueueKey(clinicId)) ?? [];
