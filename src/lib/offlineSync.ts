...
function backoffMs(failures: number) {
  return Math.min(5 * 60_000, 2_000 * Math.pow(2, Math.max(0, failures - 1))) + Math.round(Math.random() * 1000);
}
export async function runOfflineSync(clinicId: string): Promise<void> {
  ...
  try {
    ...
    if (diagnosis.code === "NO_NETWORK" || diagnosis.code === "NO_INTERNET") {
      _syncFailureCount += 1;
      _syncBackoffUntil = Date.now() + backoffMs(_syncFailureCount);
      return;
    }
...
export function registerAutomaticSync(clinicId: string, opts?: { debounceMs?: number; autoTriggerIfOnline?: boolean }) {
  ...
  const autoTrigger = opts?.autoTriggerIfOnline ?? true;
  let timer: number | null = null;
  const schedule = () => {
    if (timer) window.clearTimeout(timer);
    timer = window.setTimeout(() => {
      void runOfflineSync(clinicId);
    }, debounceMs);
  };
  ...
  const onNetworkChange = () => schedule();
  ...
  if (typeof window !== "undefined") {
    ...
    if (autoTrigger && navigator.onLine) schedule();
  }

  return {
    trigger: () => runOfflineSync(clinicId),
    unregister: () => {
      if (timer) window.clearTimeout(timer);
