...
function isUnauthorizedPostgrestError(payload: unknown) {
  if (!payload || typeof payload !== 'object') return false;
  const error = payload as PostgrestError & { status?: number; code?: string };
  return error.code === 'PGRST301' || error.code === '401' || error.status === 401 || error.status === 403;
}
...
supabase.rest['fetch'] = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (isUnauthorizedResponse(response)) {
    ...
    if (isUnauthorizedPostgrestError(payload)) {
      // eslint-disable-next-line no-console
      console.warn('[api:postgrest-unauthorized]', { url: typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url });
    }
  }
  return response;
};